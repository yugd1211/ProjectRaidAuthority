...
현재는 Mirror 룸/게임플레이 샘플을 기준으로 네트워크 구조를 잡고, 이후 실제 게임 시스템을 확장하는 단계입니다.
...
├─ 10_Mirror/                     # 외부 Mirror 플러그인, 직접 수정 지양
...
├─ 00_Scenes/                     # 프로젝트 씬
│  └─ 00_MirrorSample/            # Mirror 샘플 씬
├─ 01_Scripts/                    # 프로젝트 C# 코드
│  ├─ 00_Network/00_MirrorSample/ # Mirror 샘플 런타임 코드
│  └─ 90_Editor/                  # 에디터 빌더/검증 코드
├─ 02_Prefabs/                    # 프로젝트 프리팹
│  └─ 00_Network/00_MirrorSample/ # 네트워크 프리팹
...
```text
Assets/00_ProjectRaidAuthority/00_Scenes/00_MirrorSample/
```

## 외부 플러그인 관리 원칙

- Mirror 같은 외부 플러그인은 `Assets/10_Mirror`처럼 vendor 폴더를 유지합니다.
...
- `Assets/00_ProjectRaidAuthority/01_Scripts/90_Editor/MirrorSampleSceneBuilder.cs` - Mirror 샘플 씬 생성 코드
