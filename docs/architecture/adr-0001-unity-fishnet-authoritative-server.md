# ADR-0001: Unity + FishNet 서버 권한 기반
...
Unity 6000.4.5f1과 FishNet dedicated server를 ProjectRaidAuthority MVP의 네트워크 기반으로 사용한다.
...
직접 socket 계층 또는 Netcode for GameObjects를 먼저 채택하는 선택은 현재 팀 규모와 6개월 MVP 목표에 비해 검증 비용이 크다.
